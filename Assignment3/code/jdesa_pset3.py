...
class Bubble(InstructionGroup):
   def __init__(self, pos, r, color, octave):
...
class MainWidget4(BaseWidget) :
   ...
   def on_touch_down(self, touch) :
      ...
      self.colors = [(178, 18, 18, 1.0), (255, 252, 25, 1.0), (255, 0, 0, .5), (20,133,204, .5), (9,113,178, .5), (0,78,127, .5), (13,161,254, .5), (178, 18, 18, .5)]
      color = self.colors[interval%len(self.colors)]
      bubble = Bubble(p, r, color, octave)
...
class MainWidget5(BaseWidget) :
   def __init__(self):
      ...
      self.info = Label(text = "text", pos=(0, 480), text_size=(100,100), valign='top')
      self.add_widget(self.info)
      self.wavetype ='sine'
      self.bubbles = []
      self.audio = Audio()
      self.scaledegrees = ['1','2','3','4','5','6','7','8','9']
      self.majorkeyintervals = [0, 2, 4, 5, 7, 9, 11, 12]
      self.instructions1 = Label(text = "Tap enter to switch between interval modes. Press m for sine,  ,  for tri, . for sqare and / for sawtooth.", pos=(420, 500), text_size=(800,100), valign='top', markup=True)
   ...
   def bubble_in_collision(self,bubble1):
      ...
      for bubble2 in self.bubble_list:
         if bubble1 != bubble2:
            ...
            y2 = bubble2.pos[1]
            r2 = bubble2.radius
            distancebetween = np.sqrt((x1-x2)**2.0 + (y1-y2)**2.0) 
   ...
   def on_key_down(self, keycode, modifiers):
      if keycode[1] == 'enter':
         self.intervalmode = not self.intervalmode
      if keycode[1] == "m":
         self.setwavetype("sine")
      elif keycode[1] == ",":
         self.setwavetype("tri")
      elif keycode[1] == ".":
         self.setwavetype("square")
      elif keycode[1] == "/":
         self.setwavetype("saw")

   def setwavetype(self, wavetype):
      self.wavetype = wavetype

   def bubblecollision(self, bubble_list):
      dt = kivyClock.frametime
         ...
         for bubble2 in bubble_list:
            if bubble1 != bubble2 and not bubble1.collisionchecked and not bubble2.collisionchecked:
               ...
               if distancebetween < combinedradius:                  
                  ...
                  vect = bubble1.pos - bubble2.pos
                  f = 300000 * (combinedradius - abs(vect))
                   
                  bubble1.vel += np.sign(vect)*abs(f)*dt/(r1**2.0)
                  bubble2.vel += -np.sign(vect)*abs(f)*dt/(r2**2.0)
   ...
   def on_update(self):
         ...
         if update[1] == False:
            ...
            if bubble.collision == True:
               ...
               if self.intervalmode:
                  ...
                  gen =  NoteGenerator(60+bubble.interval, .5, .5, type = self.wavetype)
               else:
                  bubble.interval = self.majorkeyintervals[bubble.numcollisions % len(self.majorkeyintervals)]       
                  gen =  NoteGenerator(bubble.key+bubble.interval, .5, .5, type = self.wavetype)
      ...
      self.bubblecollision(self.bubble_list)
...
class PhysBubble(InstructionGroup):
   def __init__(self, pos, r, color):
      ...
      self.interval = 0
      self.key = 60 + np.random.randint(0,12)
